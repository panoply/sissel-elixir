...
var Navbar = {
  view: function view(){
    return [
      mithril('.public-navbar', {
        style: {
          transform: state.drawer.menu ? 'translateX('+state.drawer.width+'px)' : 'translateX(0px)',
          transition: 'transform 600ms cubic-bezier(0.19, 1, 0.22, 1)',
        }
      }, [
        mithril('.float-left.drawer-transition', [
          mithril('button.d-block[type="button"]', {
            onclick: function () {
            ...
            mithril('svg.public-navbar__hamburger', mithril('use[xlink:href="icons.svg#hamburger"]'))
          ]) ]),
        mithril('.float-right.drawer-transition', {
          style: {
            transform: state.drawer.menu ? 'translateX(-'+state.drawer.width+'px)' : 'translateX(0px)'
          }
        }, [
          state.authentication ?  mithril('a.pr-4.d-inline[href="/logout"]', {
            oncreate: mithril.route.link
          }, 'Logout') : mithril('a.d-inline[href="/login"]', {
            oncreate: mithril.route.link
          },[
            mithril('svg.public-navbar__login', mithril('use[xlink:href="icons.svg#login"]'))
          ])
        ])
      ])
    ];
...
var Hero = {
  view: function view(v){
    return [
      ...
      mithril(Navbar),
      mithril('.hero.row.justify-content-center.align-items-center', {
        style: {
          transform: state.drawer.menu ? 'translateX('+state.drawer.width+'px)' : 'translateX(0px)',
          overflow: state.drawer.menu ? 'hidden' : ''
        }
      },[
        mithril('.col-7.align-self-center.text-center.drawer-transition', {
          style: {
            transform: state.drawer.menu ? 'translateX(-'+state.drawer.width/2+'px)' : 'translateX(0px)'
          }
        },[
          mithril('div', v.children)
        ])
      ]),
      state.drawer.menu ? mithril('.panel-overlay', {
  ...
  view: function view() {
    return [
      ...
      mithril('.row.justify-content-center.pt-4.no-gutters', [
        ...
        mithril('.col-2', [
          mithril('svg.icon__plus', mithril('use[xlink:href="icons.svg#plus"]'))
        ]),
        mithril('.col-auto', [
          mithril('svg.icon__shopify', mithril('use[xlink:href="icons.svg#shopify"]'))
        ])
      ])
    ];
  },
...
var Auth = {
  ...
  login: function login() {
    mithril.request({
      ...
      setTimeout(function () { return Auth.error = false; }, 800);
    });
  }
};
...
var Login = {
  ...
  view: function view() {
    return [
      mithril('.row.justify-content-center', [
        mithril('.col-5', [
          mithril('.login', {
            class: Auth.error ? 'login__error' : '',
          }, [
            mithril('form.login__form', [
              mithril('input[type="email"][placeholder="Email Address"]', {
                oninput: mithril.withAttr('value', Auth.setEmail),
                value: Auth.email
              }),
              mithril('input[type="password"][placeholder="Password"]', {
                oninput: mithril.withAttr('value', Auth.setPassword),
                value: Auth.password
              }),
              mithril('.d-block.text-left.py-2', [
                mithril('a[href="/forgot-password"]', 'Forgot Password?')
              ]),
              mithril('button[type="submit"]', {
                class: Auth.error ? 'login__error' : '',
                onclick: Auth.login
              }, 'Secure Login'),
              Auth.error ? mithril('.login__error.text', 'Login Failed, Try again.') : null ])
          ])
        ])
      ])
    ];
...
mithril.route(root, '/', {
  '/': {
    render: function render() {
      return mithril(Hero, mithril(Home));
    },
  },
  '/login': {
    render: function render(){
      return mithril(Hero, mithril(Login));
    }
  },
  '/logout': {
